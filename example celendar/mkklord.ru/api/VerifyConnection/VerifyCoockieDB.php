...
class VerifyCoockieDB extends Simpla implements VerifyModelInterface
{
    const HASH_NAME = 'verify_coockie';
    const TABLE_NAME = '__connection_by_coockie';
    ...
    public function create(VerifyBaseData $info)
    {
		...
		$query = $this->db->placehold("INSERT
            INTO {$table} (hash, coockie, count, created_at, updated_at)
            VALUES ('{$info->hash}', '{$info->coockie}', {$info->count}, '{$createdAt}', '{$updatedAt}')"
        );
