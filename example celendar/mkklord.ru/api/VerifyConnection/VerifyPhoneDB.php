...
class VerifyPhoneDB extends Simpla implements VerifyModelInterface
{
    const HASH_NAME = 'verify_phone';
    const TABLE_NAME = '__connection_by_phone';
    ...
    public function create(VerifyBaseData $info)
    {
        ...
        $query = $this->db->placehold("INSERT
            INTO {$table} (hash, phone_decimal, phone, count, created_at, updated_at)
            VALUES ('{$info->hash}', '{$info->phoneDecimal}', '{$info->phone}', {$info->count}, '{$createdAt}', '{$updatedAt}')"
        );
