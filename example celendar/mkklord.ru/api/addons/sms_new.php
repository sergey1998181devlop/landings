...
define("SMSC_LOGIN", $apikeys['smsc']['login']);           // логин клиента
define("SMSC_PASSWORD", $apikeys['smsc']['password']);    // пароль
...
function send_sms_new($phones, $message, $sender = false, $translit = 0, $time = 0, $id = 0, $format = 0, $query = "", $files = array())
{
...
{
    $url = $_url = (SMSC_HTTPS ? "https" : "http")."://smsc.ru/sys/$cmd.php?login=".urlencode(SMSC_LOGIN)."&psw=".urlencode(SMSC_PASSWORD)."&fmt=1&".$arg;
...
function get_logs($phone, $start_timestamp, $cnt = 10) {
    $start = date('d.m.Y H:i:s', $start_timestamp);

    $curl = curl_init();

    curl_setopt_array($curl, array(
    CURLOPT_URL => "https://smsc.ru/sys/get.php?get_messages=1&login=".urlencode(SMSC_LOGIN)."&psw=".urlencode(SMSC_PASSWORD)."&phone=".urlencode($phone)."&cnt=".urlencode($cnt)."&fmt=3&start=".urlencode($start),
    CURLOPT_RETURNTRANSFER => true,
    CURLOPT_ENCODING => '',
