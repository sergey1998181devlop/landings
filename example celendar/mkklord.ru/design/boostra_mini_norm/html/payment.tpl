...
console.log('init')    
    ...
    app.confirm_payment = function(e){
        ...
        var insurer = $('[name=insurer]').val();
        var insure = $('[name=insure]').val();
        var prolongation = $('[name=prolongation]').val();
        var code_sms = $('[name=code_sms]').val();
        
        if ($('.payment-block').hasClass('loading'))
        {
            alert('Ожидайте завершения выполнения операции!');
            return false;
        }
        if (amount > 0)
        {            
            if ($('[name=card_id]:checked').length > 0)
            {
                ...
                if (card_id == 'other')
                {
                    app.other_payment(amount, e, prolongation, code_sms, insure, insurer);
                }
                else
                {
                    e.preventDefault();
                    app.attach_card_payment(card_id, amount, prolongation, code_sms, insure, insurer);
                }
            }
            else
            {
    ...
    app.other_payment = function(amount, e, prolongation, code_sms, insure, insurer){
        ...
        $.ajax({
            ...
            success: function(resp){
                ...
                else
                {
                    ...
                    $btn.attr('href', resp.PaymentURL);
                    $gbtn.attr('href', resp.PaymentURL);
    ...
    app.attach_card_payment = function(card_id, amount, prolongation, code_sms, insure, insurer){
...
console.info(resp);
                ...
                {
                    $('.payment-block').removeClass('loading').addClass('error');
                    $('.payment-block-error p').html('Ошибка: '+resp.error+'<br />Попробуйте выполнить платеж используя кнопку "Другая карта"');
...
console.log(resp)
                    ...
                    else
                    {
                        ...
                        else if (resp.Status == 'REJECTED')
                        {
                            ...
                            {
                                $('.payment-block').removeClass('loading').addClass('error');
                                $('.payment-block-error p').html('К сожалению во время оплаты произошла ошибка. <br />Пожалуйста введите данные карты для оплаты');
                                
                                $('[name=card_id] [value=other]').attr('checked', true);
                                $('#confirm_payment').text('Перейти к оплате');
                            }
                            else
                            {
                                $('.payment-block').removeClass('loading').addClass('error');
        ...
        <div class="payment-block">
            ...
            <div class="payment-block-main">
                ...
                <div class="payment-actions">
                    ...
                    <a href="#" target="_blank" class="button big" id="confirm_payment" type="button">Оплатить</a>
                </div>
            </div>
            
            {if $have_exitpool || $is_developer}
