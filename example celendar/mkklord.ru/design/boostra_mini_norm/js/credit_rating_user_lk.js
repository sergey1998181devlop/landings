$('#credit_rating_new_user [data-href]').on('click', function (e) {
    ...
    if (!!document.getElementsByName("is_credit_rating_page").length) {
        sendCustomMetric(4);
    }
});
...
user_cr_app.is_credit_rating_page = !!document.getElementsByName("is_credit_rating_page").length;
...
user_cr_app.send_sms = function () {
    ...
    sendMetric('reachGoal', 'new_credit_rating_get_sms_code');

    if (user_cr_app.is_credit_rating_page) {
        sendCustomMetric(5);
    }
...
user_cr_app.init_pay = function () {
    ...
    let card_id = $('[name=card_pay_id]:checked').val(),
        use_b2p = parseInt($("[name='use_b2p']").val());

    if (card_id === 'other' || !card_id) {
        user_cr_app.other_payment(user_cr_app.price, user_cr_app.code_field.val(), use_b2p);
    } else {
        user_cr_app.attach_card_payment(card_id, user_cr_app.price, user_cr_app.code_field.val(), use_b2p);
    }
};

//оплата новой картой
user_cr_app.other_payment = function(amount, sms_code, use_b2p){
    ...
    $.ajax({
        ...
        data: {
            ...
            amount: amount,
            code_sms: sms_code,
            payment_type: 'credit_rating_for_nk',
        },
        success: function(resp){
            if (!!resp.error)
            {
...
user_cr_app.attach_card_payment = function (card_id, amount, code_sms, use_b2p) {
    user_cr_app.init_spinner('Происходит попытка оплаты, подождите...');
