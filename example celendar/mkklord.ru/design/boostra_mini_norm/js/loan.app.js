function LoanApp() {
    ...
    app.init_soglasie = function () {
        $('.js-open-soglasie').click(function (e) {
            ...
            _url += 'params[birth]=' + `${day}.${month}.${year}`;
    ...
    app.init_events = function () {
        ...
        // Кнопка далее после ввода кода
        app.$form.find('.js-submit-block button').click(function () {
            _check_code();
        });
...
//        app.$form.find('input').change(_check_form)
     ...
     */
    app.initSmartCaptcha = function () {
        if (window.smartCaptcha) {
            const container = document.getElementById('smart-captcha-loan-container');

            app.widgetSmartCaptchaId = window.smartCaptcha.render(container, {
