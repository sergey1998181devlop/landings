...
{capture name=tabs}
	...
	<li {if $status==2}class="active"{/if}><a href="{url module=OrdersAdmin status=2 keyword=null id=null page=null label=null}">Выданы</a></li>
	<li {if $status==3}class="active"{/if}><a href="{url module=OrdersAdmin status=3 keyword=null id=null page=null label=null}">Отказы</a></li>
	<li {if $status==4}class="active"{/if}><a href="{url module=OrdersAdmin status=4 keyword=null id=null page=null label=null}">Удалены</a></li>
	{if $keyword}
...
{* Title *}
{$meta_title='Заявки' scope=parent}
...
{* Заголовок *}
<div id="header">
	<h1>{if $orders_count}{$orders_count}{else}Нет{/if} заяв{$orders_count|plural:'':'ок':'ки'}</h1>		
	<a class="add" href="{url module=OrderAdmin}">Добавить заявку</a>
</div>	
...
<div id="main_list">
	...
	<input type="hidden" name="session_id" value="{$smarty.session.id}">
		<div id="list">		
			...
			<div class="{if $order->paid}green{/if} row">
				...
				<div class="order_date cell">				 	
	 				...
	 				<br/>
	 				{if $order->utm_source}<b>{$order->utm_source}</b>{/if}
				</div>
				<div class="order_name cell">
					...
					<a href="{url module=OrderAdmin id=$order->id return=$smarty.server.REQUEST_URI}">
						№{$order->id} - {$order->user->Faktcity} {if $order->id_1c}(1c - {$order->id_1c}){/if}
					</a>
					<br/><br/>
					<a href='index.php?module=UserAdmin&id={$order->user->id}' target=_blank>
						{$order->user->lastname} {$order->user->firstname} {$order->user->patronymic}
					</a>
					<br/>
					{$order->user->phone_mobile|escape}
					{if $order->note}
					<br/>
				...
				<div class="name cell" style='white-space:nowrap;'>
	 				{$order->amount|escape} {$currency->sign} - {$order->period|escape} дней
				</div>
				...
				<div class="icons cell">
						...
						{/if}
						{if $order->status == 2}
						<img src='design/images/tick.png' alt='Выдан' title='Выдан'>
						{/if}
						{if $order->status == 3}
						<img src='design/images/cross.png' alt='Отказ' title='Отказ'>
						{/if}
						{if $order->status == 4}
		...
		<select name="action">
			...
			{if $status!==2}<option value="set_status_2">В выданные</option>{/if}
			{if $status!==3}<option value="set_status_3">В отказ</option>{/if}
			{foreach $labels as $l}
			...
			{/foreach}
			<option value="delete">Удалить выбранные заявки</option>
		</select>
		</span>
...
<div id="right_menu">
	...
	<!-- Метки -->
	<ul id="labels">
		<li {if !$label}class="selected"{/if}><span class="label"></span> <a href="{url label=null}">Все заявки</a></li>
		{foreach $labels as $l}
