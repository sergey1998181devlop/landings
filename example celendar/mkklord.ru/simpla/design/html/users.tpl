...
{* Title *}
{$meta_title='Заемщики' scope=parent}
...
<div id="search">
	...
	<input class="search" type="text" name="keyword_phone" value="{$keyword_phone|escape}" placeholder="Телефон" autocomplete="off"/><br/>
	<input class="search" type="text" name="keyword_surname" value="{$keyword_surname|escape}" placeholder="Фамилия" autocomplete="off"/><br/>
	<input class="search" type="text" name="keyword_name" value="{$keyword_name|escape}" placeholder="Имя" autocomplete="off"/><br/>
	<input class="search" type="text" name="keyword_patronimic" value="{$keyword_patronimic|escape}" placeholder="Отчество" autocomplete="off"/><br/>
...
<div id="main_list">
	...
	<input type="hidden" name="session_id" value="{$smarty.session.id}">
		...
		<div id="list">	
			...
			<div class="{if !$user->enabled}invisible{/if} row">
				...
				<div class="user_name cell">
					<a href="index.php?module=UserAdmin&id={$user->id}">{$user->lastname|escape} {$user->firstname|escape} {$user->patronymic|escape}</a>	
					<br/><br/>
					UID: {$user->uid|escape}
					<br/>
					UID_status: {$user->uid_status|escape}
					<br/>
					<br/>
					<a href="/user?user_id={$user->id}" target="_blank">Войти в ЛК</a>
				</div>
				<div class="user_email cell">
					<a href="index.php?module=UserAdmin&id={$user->id}">{$user->phone_mobile|escape}</a>
					<br/><br/>
					{if $user->balance}
						Займ номер: {$user->balance->zaim_number}<br/>
						Основной долг: {$user->balance->ostatok_od}<br/>
						Остаток Процента: {$user->balance->ostatok_percents}<br/>
						Остаток пени: {$user->balance->ostatok_peni}<br/>
						Итого: {$user->balance->ostatok_od+$user->balance->ostatok_percents+$user->balance->ostatok_peni}
					{/if}
				</div>
				<div class="user_group cell">
					{if $user->group_id}{$groups[$user->group_id]->name} | {/if}{$user->Faktcity|escape}
				</div>
		...
		<span id=select>
		<select name="action">
			<option value="update_balance">Обновить баланс</option>
