...
# Rename the files and generate the HTML links
sorted_keys = sorted(file_mappings.keys())
...
for old_name in sorted_keys:
    ...
    display_name = old_name.split('.pdf')[0]
    if display_name[0].isdigit() and display_name[1] == '.':
        display_name = display_name[2:].strip()

    if os.path.exists(old_path):
        os.rename(old_path, new_path)
        print(f"Renamed '{old_name}' to '{new_name}'")
        link = f'<a target="_blank" href="/files/docs/{new_name}">{display_name}</a><br>'
